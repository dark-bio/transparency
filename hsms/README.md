...
## YubiKey authenticity attestations
...
Unlike YubiHSMs, the YubiKey device attestations do not contain the serial number, those can be cross-referenced from key attestations.

## Audit logs

Each HSM has a pre-annotated audit log published. You can extract the raw YubiHSM audit logs from it with [`./scripts/extract.sh`](./scripts/extract.sh). The log hash progression can be verified either via a third-party tool, or with [`./scripts/verify.sh`](./scripts/verify.sh) included in this repository.
...
Unfortunately, YubiHSM 2 with firmware 2.4 does ***not*** support signing its own audit log. As such, while the above scripts can verify audit log hash progressions; they can attest neither the provenance of the audit log (i.e. which YubiHSM generated it, if any) nor its freshness (i.e. are there unpublished entries).

The only cryptographically secure way to validate a YubiHSM audit log is to establish a live session via asymmetric-key authentication (forcibly verifying the identity of the YubiHSM) and then query the audit log live.
...
Keeping the HSMs public at all times isn't a sane proposition, so for audit queries, please contact us at transparency@dark.bio, and we'll try to grant you temporary access. Note that we will only respond to queries from legitimate sources, and we reserve the right to publish the request and resolution.
